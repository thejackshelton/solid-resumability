/**
 * The rule page's artifact registry: the one emitted directory whose stem
...
const STATIC_MODULES = import.meta.glob<Record<string, unknown>>(
  ["../artifacts/*.SeparatorRoot/structure.js", "../artifacts/*.SeparatorRoot/wiring.js"],
  { eager: true },
);

const HANDLER_MODULES = import.meta.glob<HandlerModule>("../artifacts/*.SeparatorRoot/handlers/*.js");

export const registry = createRegistry(STATIC_MODULES, HANDLER_MODULES);

/** Resumes the rule page's one component from this build's artifacts. */
